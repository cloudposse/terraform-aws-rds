...
variable "ssm_key_format" {
  type        = string
  default     = "/%v/%v/%v"
  description = "SSM path format. The values will will be used in the following order: `var.ssm_key_prefix`, `var.database_name`, `var.ssm_key_*`"
}
...
resource "aws_ssm_parameter" "rds_database_user" {
  ...
  name        = format(var.ssm_key_format, var.ssm_key_prefix, var.database_name, var.ssm_key_user)
...
resource "aws_ssm_parameter" "rds_database_password" {
  ...
  name        = format(var.ssm_key_format, var.ssm_key_prefix, var.database_name, var.ssm_key_password)
...
resource "aws_ssm_parameter" "rds_database_hostname" {
  ...
  name        = format(var.ssm_key_format, var.ssm_key_prefix, var.database_name, var.ssm_key_hostname)
...
resource "aws_ssm_parameter" "rds_database_port" {
  ...
  name        = format(var.ssm_key_format, var.ssm_key_prefix, var.database_name, var.ssm_key_port)
...
output "rds_database_ssm_key_prefix" {
  value       = local.ssm_enabled ? format(var.ssm_key_format, var.ssm_key_prefix, var.database_name, "") : null
